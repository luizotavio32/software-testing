...
class TranspilingLanguageService {
    constructor(compilerOptions, rootFiles, projectDirectory, produceSourceMaps, getLogger) {
        ...
        rootFiles.forEach((file) => (this.files[tsHelpers_1.normalizeFileForTypescript(file.name)] = new ScriptFile_1.default(file.name, file.textContent)));
    ...
    replace(replacements) {
        replacements.forEach((replacement) => this.files[tsHelpers_1.normalizeFileForTypescript(replacement.name)].replace(replacement.textContent));
    }
     ...
     */
    emit(fileName) {
        const emittedFiles = this.languageService.getEmitOutput(tsHelpers_1.normalizeFileForTypescript(fileName)).outputFiles;
