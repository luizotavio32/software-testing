...
class TypescriptTranspiler {
    constructor(options, produceSourceMaps, getLogger) {
        ...
        this.filter = TranspileFilter_1.default.create(options);
    }
    transpile(files) {
    ...
    transpileFiles(files) {
        ...
        files.forEach((file) => {
            if (!tsHelpers_1.isHeaderFile(file.name) && !this.hasJsonOutput(file.name)) {
    ...
    hasJsonOutput(fileName) {
        return fileName.endsWith('.json') && !tsHelpers_1.getTSConfig(this.options).options.outDir;
    }
}
